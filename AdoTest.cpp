...
int sample_procedure_call()
{
	...
	TCHAR charVal[MAX_STR_LEN] = _T("abcdefghij");
	COleDateTime smallDateTimeVal(2016, 03, 07, 11, 12, 11), dateTimeVal(2016, 03, 07, 11, 12, 11);
	BYTE binaryVal[] = { 0X01, 0X02, 0X03, 0X04, 0X05, 0X06, 0X07, 0X08, 0X09, 0X0A };
	...
	try
	{
		...
		pDB->CreateParameter(_T("@io_charVal"), adParamInputOutput, charVal);
		...
		pDB->CreateParameter(_T("@io_binaryVal"), adParamInputOutput, binaryVal);
		...
		while (pDB->HasNext())
		{
			...
			pDB->GetFieldValue(_T("charVal"), charField);
			pDB->GetFieldValue(_T("datetimeVal"), dateTimeField);
			pDB->GetFieldValue(_T("binaryVal"), binaryValField, nSize);
		...
		pDB->GetParameter(_T("@io_charVal"), charVal);
		pDB->GetParameter(_T("@io_binaryVal"), binaryVal, nSize);
...
int sample_raw_query()
{
	try
	{
		...
		pDB->SetQuery(_T("Insert Into sample_table Values(1, 127, 20000, 12345678, 888888888888, 3.14, 'abcdefghij', '20160308', '20160308', null, null, null, null)"));
